...
from dptools.net import torproxyservice
...
class TorProxyServiceClassSetup(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tor_key_dir_val = self.temp_dir_path
        self.tor_controller_port = 9051
        self.x = torproxyservice.TorProxyService(self.name,
                                                 self.port,
                                                 self.q,
                                                 tor_key_dir=self.tor_key_dir_val,
                                                 tor_controller_port=self.tor_controller_port)
        self.mock_tor_data_dir_path = self.temp_dir_path + self.x.name
...
class TestTorProxyServiceClassBasics(TorProxyServiceClassSetup):
    ...
    def test_tor_proxy_error_string_atrr_set(self):
        error_string = 'Tor_Proxy_Error'
        self.assertEqual(error_string, self.x.tor_proxy_error)
    ...
    def test_build_error_payload_method_builds_error_payload_from_arg(self):
        ...
        target_dict = dict(desc=self.x.tor_proxy_error, name=self.x.name, error=target_string)
    ...
    @patch('net.torproxyservice.stem.socket')
...
class TestTorProxyServiceClassPersistentSlow(TorProxyServiceClassSetup):
    ...
    @patch.object(torproxyservice, 'Controller')
    @patch('net.torproxyservice.stem.socket')
    def test_start_persistent_method_catches_controller_error_create_ephemeral(self, socket, controller):
        sub = self.x.run_check = MagicMock()
        sub_write = self.x._write_key_file = MagicMock()
