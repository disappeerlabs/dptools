...
class TorProxyService:
    ...
    tor_proxy_error = 'Tor_Proxy_Error'

    def __init__(self, name, port, queue, tor_key_dir=None, tor_controller_port=9051):
        self.name = name
        self.port = port
        self.queue = queue
        self.tor_key_dir = tor_key_dir
        self.controller_port = tor_controller_port
        self.running = True
    ...
    def build_error_payload(self, error_msg):
        payload = dict(desc=self.tor_proxy_error, name=self.name, error=error_msg)
        return payload
